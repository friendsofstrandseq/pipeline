...
This mode disables the segmentation part and gets the user-defined segmentation as an input bed file. For running the pipeline in this mode, set `manual_segments` to `true` in the `Snake.config.json` file, then create a subdirectory `manual_segmentation` and add you bed file named `sampleName.bed` there:
   ```
   cd pipeline
