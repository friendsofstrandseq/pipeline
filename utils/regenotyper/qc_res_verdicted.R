...
make_barplot_invwise <- function(rv_f, samples_f){
  ...
  simple_events_lowconf = c('0/0_lowconf', '0|0_lowconf', '0|1_lowconf', 
  ...
  res_verdict = rv_f %>% mutate_all(funs(ifelse(. %in% simple_events, 'simple', .))) %>% 
  ...
  res_verdict_molten = melt(res_verdict[,c('invno','verdict',samples)], id.vars = c('invno', 'verdict'))
  
  ### PLOT ###
  p2 = ggplot() + geom_bar(data=res_verdict_molten, aes(x=as.character(invno), fill=value)) + 
    scale_fill_manual(values=c("blue", "white",'darkgrey', "darkgreen", "green")) +
    xlim(res_verdict_verdictsorted) 
  
  p3 = p2 +  geom_segment(aes(y=baseheight, 
                              yend = baseheight, 
    ...
    geom_text(aes(x=((xpos[2:length(xpos)]-xpos[1:length(xpos)-1])/2)+xpos[1:length(xpos)-1], 
...
make_barplot_groupwise <- function(idf_f){
  ...
  idf_2 = idf_f %>% group_by(simpleGT) %>% mutate(sum_events = sum(value)) %>% slice(1)
  p2 = ggplot(idf_2) + geom_bar(aes(x=simpleGT, y=sum_events, fill=simpleGT), stat='identity') + 
...
make_barplot_all <- function(idf_f){
  ...
  p1 = ggplot(idf_f) + geom_bar(aes(x=variable, y=value, fill=simpleGT), stat='identity') + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
    scale_x_discrete(rev(xorder)) 
...
make_inventory <- function(rv_f, samples_f){
  ...
  invent = table(unlist(rv_f[,samples_f]))
