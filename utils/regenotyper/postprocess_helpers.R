...
count_homhetrefetc <- function(cm_f, n_samples){
  ...
  cm_f$nduphom = rowSums2(cm_f == "2020") + rowSums2(cm_f=='2020_lowconf')
  cm_f$nnoreads = rowSums2(cm_f=='noreads')
  
  ##dealing with INVDUPs separately
  ...
  ##Counting complex calls. Invdups are not counted as such.
...
apply_filter_new <- function(cm_f, samples){
  ...
  cm_f$FP = cm_f$nref + cm_f$nduphom == length(samples)
