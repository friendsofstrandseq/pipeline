...
def parse_command_line():
    ...
    parser.add_argument('--input', '-i', type=str, dest='input', required=True,
                        help='Gzipped, tab-separated table with Watson/Crick read '
                             'counts in fixed bins. A header line is required.')
    parser.add_argument('--output', '-o', type=str, dest='output', required=True,
                        help='Full path to output text file.')
    parser.add_argument('--dump-table', '-tab', type=str, dest='table', default='',
                        help='Specify a path to a file to dump the table of '
                             ...
                             'Default: <none>')
    parser.add_argument('--blacklist-regions', '-b', type=str, dest='blacklist', default='',
                        help='Specify file with regions to be blacklisted. Only '
                             'chrom - start - end will be read from the file. '
                             'Default: <none>')
    parser.add_argument('--max-ploidy', default=4, type=int, dest='max_ploidy',
                        help='Maximum ploidy that is considered. '
                             ' Default: 4')
    parser.add_argument('--boundary-alpha', '-a', type=float, default=0.05, dest='alpha',
                        help='Adjust means of Gaussians at the boundaries (0, 1) by alpha '
                             'to account for a some noise in the data (imperfect ratios). '
                             'Default: 0.05')
    parser.add_argument('--merge-bins-to', '-m', type=int, default=1000000, dest='window',
                        help='Merge the input bins to windows of this size. '
                             'Default: 1000000')
    parser.add_argument('--shift-window-by', '-s', type=int, default=500000, dest='step',
                        help='Shift merge window by this step size. '
                             'Default: 500000')
    parser.add_argument('--uniform-background', '-ubg', action='store_true',
                        ...
                        help='Add an additional component to the mixture '
                             ...
                             'Default: False')
    parser.add_argument('--sort-input', '-si', action='store_true', default=False, dest='sort',
                        help='Set this option if the input data is NOT sorted by: '
                             'cell > chrom > start > end '
                             'Default: False')
    parser.add_argument('--jobs', '-j', default=1, type=int, dest='jobs',
...
class Mixture:
    ...
    def fit_stddevs_meanprop(self, fractions):
        ...
        while True:
            for idx, (mean, stddev, weight) in enumerate(zip(self.means,
                                                             self.stddevs,
                                                             self.weights)):
...
def compute_watson_fractions(input_file, window_size, shift_size, not_sorted, jobs):
    ...
    if not_sorted:
        ...
        df.sort_values(['cell', 'chrom', 'start', 'end'],
    ...
    subsets_to_process = [(select_range,
                           shift_step,
                           window_size,
                           subset_id,
                           subset_counts) for subset_id, subset_counts in df.groupby(['cell', 'chrom'])]
    ...
    assert len(sanity_checks.keys()) == len(cells_in_data), \
        'Missing data for cell(s): {}'.format(sorted(cells_in_data - set(sanity_checks.keys())))
    for cell, chroms in sanity_checks.items():
        assert len(chroms) == len(chrom_in_data), \
            'Missing chromosome(s) for cell {}: {}'.format(cell, chrom_in_data - chroms)
...
def check_bin_window_compatibility(count_data, window_size, shift_size):
    ...
    if not count_data.at[0, 'end'] <= count_data.at[1, 'start']:
        raise ValueError('Input bins are overlapping: '
                         '(0) {} > {} (1)'.format(count_data.at[0, 'end'],
                                                  count_data.at[1, 'start']))

    if not window_size % bin1 == 0:
        raise ValueError('User-specified merge window size is not a '
                         'multiple of input bin size: {} mod {} != 0'.format(window_size, bin1))
    ...
    if not shift_size % bin1 == 0:
        raise ValueError('User-specified window shift step is not a '
                         'multiple of input bin size: {} mod {} != 0'.format(shift_size, bin1))
        ...
        for begin in range(0, count_data.shape[0], shift_step):
            ...
            try:
                ...
                if w_fraction == -1 or np.isnan(w_fraction):
                    ...
                    raise
            assert not np.isnan(w_fraction), \
                'NaN w_fraction in {}: {} / {}'.format(subset_id,
                                                       begin,
                                                       begin+select_range)
            assert 0 <= w_fraction <= 1, \
                'Out-of-bounds w_fraction in {}: {} / {} / {}'.format(subset_id,
                                                                      w_fraction,
                                                                      begin,
                                                                      begin+select_range)
...
def main():
    ...
    frac_dataset = compute_watson_fractions(
        args.input,
        args.window,
        args.step,
        args.sort,
    ...
    ploidy_estimates = run_ploidy_estimation(
        frac_dataset,
        args.max_ploidy,
        args.alpha,
        args.background,
